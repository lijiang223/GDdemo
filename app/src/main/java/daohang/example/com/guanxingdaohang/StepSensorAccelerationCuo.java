 ...
 * 加速度传感器
 */
public class StepSensorAccelerationCuo extends StepSensorBase {
    ...
    private long timeO = 0;

    public StepSensorAccelerationCuo(Context context, StepCallBack stepCallBack) {
        super(context, stepCallBack);
    }

    @Override
    protected void registerStepListener(float[] a) {
        // 注册加速度传感器
...
//            Log.i(TAG, "加速度传感器不可用！");
//        }
        if(System.currentTimeMillis()-timeO>2000){
            average = (float) Math.sqrt(Math.pow(a[0], 2)
        ...
        if (gravityOld == 0) {
            ...
            if (DetectorPeak(values, gravityOld)) {
                ...
                if (timeOfNow - timeOfLastPeak >= 200
                        && (peakOfWave - valleyOfWave >= ThreadValue) && (timeOfNow - timeOfLastPeak) <= 10000) {
